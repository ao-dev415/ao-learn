...
(site.chapters||[]).forEach((ch,ci)=>{
  (ch.los||[]).forEach((lo,li)=>{
    const id = `ch${ci+1}.lo${li+1}`;
    const url = `#ch${ci+1}-lo${li+1}`;
    const title = lo.title || `Learning Objective ${ci+1}.${li+1}`;
    const parts = [];
    if (lo.preview) parts.push(plain(lo.preview));
    (lo.snippets||[]).forEach(s=>{ if(s?.title) parts.push(plain(s.title)); if(s?.body) parts.push(plain(s.body)); });
